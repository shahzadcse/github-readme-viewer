...
export function Header(props: HeaderProps) {
  ...
  useEffect(() => {
    async function getAuxUserData() {
      await api.get(`users/${props.auxUser}`).then((response) => {
  ...
  function HeaderIcons() {
    if (props.user !== null) {
      return (
        <>
          <RiLogoutCircleLine color={"#A51B0B"} onClick={props.logOut} />
      ...
      return (
        <>
          <BiArrowBack onClick={() => navigate(`/${props.lastPage}`)} />
  ...
  function UserAvatar() {
    if (props.user !== null) {
      return <img src={props.user.avatar_url} alt="User Avatar - Github" />;
    } else {
      return <img src={auxUserData?.avatar_url} alt="User Avatar - Github" />;
    }
  }
